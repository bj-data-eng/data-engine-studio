...
impl CreateParentTestNode for (f32, f32, i32, i32) {
    fn into_grid(self) -> Style {
        Style {
            ...
            size: Size {
                width: Dimension::from_length(self.0),
                height: Dimension::from_length(self.1),
            },
...
impl CreateExpectedPlacement for (i16, i16, i16, i16) {
    fn into_oz(
        ...
        (
            OriginZeroLine(self.0),
            OriginZeroLine(self.1),
            OriginZeroLine(self.2),
            OriginZeroLine(self.3),
        )
    }
}
