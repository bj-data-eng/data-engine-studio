use des_graph_egui::node::EdgeEvent;
use des_graph_egui::SocketKind;
use eframe::egui;
...
struct State {
    ...
    auto_layout: bool,
    node_spacing: [f32; 2],
    node_id_map: HashMap<des_graph_egui::NodeId, NodeIndex>,
    center_view: bool,
    dot_grid: bool,
    immutable: bool,
}
...
fn layout(
    ...
    node_spacing: [f32; 2],
    ctx: &egui::Context,
) -> des_graph_egui::Layout {
    let graph_id = graph_id();
    // Access graph memory once and iterate inside to avoid repeated locks
    let nodes_vec = des_graph_egui::with_graph_memory(ctx, graph_id, |gmem| {
        let node_sizes = gmem.node_sizes();
        graph
            .node_indices()
            .map(|n| {
                let node_id = des_graph_egui::NodeId::from_u64(n.index() as u64);
                let size = node_sizes
                    .get(&node_id)
                    .cloned()
    ...
    let edges = graph
        ...
        .map(|(a, b)| {
            (
                des_graph_egui::NodeId::from_u64(a.index() as u64),
                des_graph_egui::NodeId::from_u64(b.index() as u64),
            )
        });
    let mut layout = des_graph_egui::layout(nodes, edges, flow);
...
fn gui(ui: &mut egui::Ui, view: &mut des_graph_egui::View, state: &mut State) {
...
fn graph(ui: &mut egui::Ui, view: &mut des_graph_egui::View, state: &mut State) {
    let graph_response = des_graph_egui::Graph::from_id(graph_id())
...
fn nodes(nctx: &mut des_graph_egui::NodesCtx, ui: &mut egui::Ui, state: &mut State) {
    ...
    for n in indices {
        ...
        let response = des_graph_egui::node::Node::from_id(node_id)
            .inputs(inputs)
            .outputs(outputs)
            .flow(state.flow)
...
fn edges(ectx: &mut des_graph_egui::EdgesCtx, ui: &mut egui::Ui, state: &mut State) {
    ...
    for e in state.graph.edge_indices().collect::<Vec<_>>() {
        ...
        let a = des_graph_egui::NodeId::from_u64(na.index() as u64);
        let b = des_graph_egui::NodeId::from_u64(nb.index() as u64);
        let mut selected = state.interaction.selection.edges.contains(&e);
        let response = des_graph_egui::edge::Edge::new((a, output), (b, input), &mut selected)
...
fn graph_config(ui: &mut egui::Ui, view: &mut des_graph_egui::View, state: &mut State) {
